//Codice di gubbo del mio compleanno

//https://github.com/Mavik7zip/lora-esp8266-test/blob/main/main/main.cpp
...
void print_display(strt_pkt packet) {
  ...
  display.print(F("Rssi: ")); display.print(packet.rssi);
  display.setCursor(0, 30);
  display.print(F("Snr: ")); display.print(packet.snr);
  display.display();
}
...
void read_packet(int packet_size) {
  ...
  for (int i = 0; i < packet_size; i++) { //inizio lettura pacchetto
    tmp_char = (char)LoRa.read();

    if (tmp_char != '\n') {               //verifica carattere di stop
      tmp += tmp_char;
    } else {
      switch (j) {                        //bho
        case 0:
